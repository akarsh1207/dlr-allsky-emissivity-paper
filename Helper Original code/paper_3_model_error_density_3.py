...
# 已知最佳参数 (c_1, c_2) for Model3
best_c1 = 0.0790
best_c2 = -0.3221

X_model3 = df_all[['k_t_dni', 'sqrt_pw', 'altitude']].values
y_all = df_all['e_sky'].values


def predict_model3(X, c_1, c_2):
    k_t_dni = X[:, 0]
    sqrt_pw = X[:, 1]
    altitude = X[:, 2]
    CF = c_1 * (k_t_dni ** c_2)
...
# 网格范围： c_1 in [-0.5,0.5], c_2 in [-1,0]
c1_vals3 = np.linspace(0.05, 0.25, 10)
c2_vals3 = np.linspace(-0.5, -0.1, 10)

RMSE_map3 = rmse_on_grid(X_model3, y_all, predict_model3, c1_vals3, c2_vals3)
best_rmse = compute_rmse(y_all, predict_model3(X_model3, best_c1, best_c2))

plt.figure(figsize=(5.5, 5))
contour = plt.contourf(c2_vals3, c1_vals3, RMSE_map3, levels=10, cmap='coolwarm')
plt.plot(best_c2, best_c1, '^', color='black', markersize=8)
plt.text(best_c2, best_c1, f"({best_c1:.3f}, {best_c2:.3f})\nRMSE: {best_rmse:.4f}",
         ha='left', va='bottom', fontsize=10)
plt.xlabel('$c_2$')
plt.ylabel('$c_1$')
plt.title('Model3 RMSE Contour')
plt.colorbar(contour, label='RMSE')
plt.tight_layout()
plt.show()
# plt.savefig(os.path.join(output_dir_all, "model3_rmse_contour.png"), dpi=300, bbox_inches='tight')
